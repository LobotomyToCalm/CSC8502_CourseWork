...
//
//Mesh* Mesh::GenerateRain(float width, float height) {
//	Mesh* m = new Mesh();
//	m->numVertices = 10000;
//
//	m->vertices = new Vector3[m->numVertices];
...
//	m->colours = new Vector4[m->numVertices];
//	for (int i = 0; i < numVertices, i++) {
//		vertices[i] = Vector3(rand() % (int)width, 150.0f, rand() % (int)height);
//		colours[i] = Vector4(1.0, 1.0, 1.0, 1.0);
//	}
//
//	m->BufferData();
//	return m;
...
void Mesh::GenerateTangents() {
	...
	auto size1 = sizeof(tangents);
	auto size2 = sizeof(textureCoords);
	for (int i = 0; i < triCount; ++i) {
		unsigned int a = 0;
		unsigned int b = 0;
		unsigned int c = 0;
...
// Use for generate Low Poly triangles corners, conflict with GenerateTangents()!!!
void Mesh::GenerateLowPolyCorners() {
	// Set coords of another two Corner of a triangle 
	// First two is right corner,second one is left corner
	for (int i = 0; i < numVertices; i++) {
		Vector2 cornerA, cornerB;
		if (i % 3 == 0) {
			cornerA = Vector2(vertices[i + 1].x, vertices[i + 1].z);
			cornerB = Vector2(vertices[i + 2].x, vertices[i + 2].z);
		}
		if (i % 3 == 1) {
			cornerA = Vector2(vertices[i + 1].x, vertices[i + 1].z);
			cornerB = Vector2(vertices[i - 1].x, vertices[i - 1].z);
		}
		if (i % 3 == 2) {
			cornerA = Vector2(vertices[i - 2].x, vertices[i - 2].z);
			cornerB = Vector2(vertices[i - 1].x, vertices[i - 1].z);
		}

		tangents[i] = Vector4(cornerA.x, cornerA.y, cornerB.x, cornerB.y);
	}
